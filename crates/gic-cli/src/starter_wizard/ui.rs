...
use gic_core::starter_engine::models::{ProjectConfig, ProjectType, Question, QuestionType, TemplateGenerator};
...
pub fn run_wizard(path: Option<&std::path::Path>, project_type: ProjectType) -> Result<Option<ProjectConfig>> {
    enable_raw_mode()?;
    ...
    while !finished && !canceled && current_q_idx < active_questions.len() {
        ...
        terminal.draw(|f| {
            ...
            let title = Paragraph::new(Line::from(vec![
                Span::styled("GIC Project Starter Engine", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
            ]))
            .block(Block::default().borders(Borders::ALL));
            f.render_widget(title, chunks[0]);

            let content_block = Block::default().borders(Borders::ALL).title(q.prompt.clone());
            ...
            match &q.q_type {
                ...
                QuestionType::Text { default } => {
                    let display_text = if text_input.is_empty() {
                        format!("{} (default: {})\n\n[Type your answer and press Enter to submit]", text_input, default)
                    } else {
        ...
        ProjectType::Terraform => vec![
            ...
            Question {
                ...
                condition: Some(|config| {
                    config.get_answer("cloud").map(|k| k != "Manual (Empty File)").unwrap_or(true)
                }),
            },
        ],
        ProjectType::Ansible => vec![
            Question {
