...
use gic_core::{
    CursorPosition, Document, EditorMode, EditorState, InputEvent, KeyCode, ShutdownReason, TextBuffer,
    UIConfig,
};
use gic_tui::{
    ...
    renderer::{
        pipeline::RenderPipeline, render_state::RenderState as TuiRenderState, themes::builtin, viewport::Viewport,
    },
...
impl EditorApp {
    ...
    pub fn new(file_path: Option<PathBuf>, ui_config: UIConfig, debug_mode: bool) -> Self {
        ...
        let project_root = file_path.as_ref().and_then(|p| gic_core::workspace::detect_project_root(p));
        ...
        let display_name = state.document()
            .path
            .as_ref()
    ...
    fn update_diagnostics(&mut self) {
        let file_name = self.state.document().path.as_ref().and_then(|p| p.file_name()).and_then(|n| n.to_str()).unwrap_or("").to_string();
        let file_ext = self.state.document().path.as_ref().and_then(|p| p.extension()).and_then(|e| e.to_str()).unwrap_or("").to_string();
        ...
        // Run spell checker
        let spell_diags = gic_core::language_engine::spell_checker::check_spelling(&text, engine.id());
        ...
        if engine.id() == "kubernetes" && text.contains("kind: Deployment") {
            if !text.contains("kind: Service") {
                self.cached_diagnostics.push(gic_core::language_engine::EngineDiagnostic {
                    ...
                    message: "Missing: Service, Ingress. [Press Alt+G to Generate Missing Resources]".to_string(),
            ...
            let has_compose = if let Some(path) = &self.state.document().path {
                ...
                if let Some(parent) = p.parent() {
                    parent.join("docker-compose.yml").exists() || parent.join("compose.yml").exists()
                } else {
                    false
                }
            } else {
        ...
        self.cached_diagnostics.sort_by(|a, b| a.row.cmp(&b.row).then(a.col.cmp(&b.col)));
    }
    ...
    fn jump_to_next_placeholder(&mut self) -> bool {
        ...
        if let Some((r, c)) = found {
            ...
            buffer.selection_mut().start(gic_core::CursorPosition::new(r, c), gic_core::buffer::selection::SelectionMode::Character);
            buffer.selection_mut().update(gic_core::CursorPosition::new(r, c + 1));
    ...
    fn apply_quick_fix(&mut self) -> bool {
        ...
        if !self.state.quick_fix_menu_open {
            ...
            if let Some(diag) = self.cached_diagnostics.iter().find(|d| d.row == cursor.row && !d.quick_fixes.is_empty()) {
                ...
                self.state.engine.set_status("No Quick Fix available for this line".to_string());
                return false;
            }
        }

        // Apply the chosen fix
        let fix = if let Some(diag) = self.cached_diagnostics.iter().find(|d| d.row == cursor.row && !d.quick_fixes.is_empty()) {
        ...
        self.state.engine.set_status(format!("✓ Applied Quick Fix: {}", fix.title));
        return true;
    }
    ...
    pub fn run(mut self) -> Result<()> {
        ...
        let mut viewport = Viewport::new(initial_text_height, w as usize, self.state.buffer_mut().line_count());
        ...
        let shutdown_reason = loop {
            if render_engine.should_render(&self.state.dirty) {
                ...
                let language = self.state.document().path.as_ref().map(|p| detect_language(p)).unwrap_or("Plain Text");
    ...
    fn handle_key_input(&mut self, code: KeyCode, modifiers: gic_core::KeyModifiers) -> Option<ShutdownReason> {
        let control = modifiers.control;
        let alt = modifiers.alt;
        
        if control && code == KeyCode::Char('q') {
            let is_modified = self.state.document_mut().is_modified || self.state.buffer_mut().is_modified();
                ...
                if let Some(code) = &diag.code {
                    if code == "GIC-SUGGEST" {
                        if diag.message.contains("Service, Ingress") {
                            ...
                            self.state.dirty.mark_full();
                            self.sync_cursor_from_buffer();
                            self.state.engine.set_status("Generated Missing Resources: Service, Ingress".to_string());
                            ...
                            if let Some(path) = &self.state.document().path {
                                ...
                                if let Some(parent) = p.parent() {
                                    ...
                                    let _ = std::fs::write(parent.join(".dockerignore"), "node_modules\ntarget\n.git\n");
        ...
        match self.state.mode {
            EditorMode::Search => match code {
                ...
                KeyCode::Char(ch) => {
                    ...
                    self.state.engine.set_status(format!("Search: {}", self.state.search_query));
                    self.perform_search();
                }
                KeyCode::Backspace => {
            ...
            EditorMode::Normal => match code {
                ...
                KeyCode::Char('o') => {
                    ...
                    let pos = self.state.buffer_mut().cursor_position(); self.state.set_cursor(pos);
                    self.state.document_mut().mark_modified();
                    self.state.set_mode(EditorMode::Insert, "-- INSERT --");
                ...
                KeyCode::F(8) => {
                    ...
                    if let Some((row, col, severity)) = next_diag {
                        ...
                        self.state.dirty.mark_full();
                        self.update_hover();
                        self.state.engine.set_status(format!("Jumped to {:?} (F8)", severity));
                    } else {
                        self.state.engine.set_status("No diagnostics in file.".to_string());
                    }
                }
                ...
                KeyCode::Char('j') | KeyCode::Down => {
                    if self.state.quick_fix_menu_open {
                        if let Some(diag) = self.cached_diagnostics.iter().find(|d| d.row == self.state.cursor().row && !d.quick_fixes.is_empty()) {
                ...
                KeyCode::Char('x') => {
                    ...
                    if self.state.buffer_mut().delete_char().is_ok() {
                        let pos = self.state.buffer_mut().cursor_position(); self.state.set_cursor(pos);
                        self.state.document_mut().mark_modified();
                        self.state.engine.set_status("Character deleted".to_string());
            ...
            EditorMode::Insert => match code {
                ...
                KeyCode::Char(ch) => {
                    ...
                    let cursor = self.state.cursor();
                    let next_char = self.state.buffer_mut().lines().get(cursor.row)
                ...
                KeyCode::Enter => {
                    if !self.cached_completions.is_empty() {
                        let comp = self.cached_completions[self.state.autocomplete_selected_index].clone();
                        self.cached_completions.clear();
                        self.cached_ghost_text = None;
                    ...
                    let ext = path.as_ref().and_then(|p| p.extension()).and_then(|e| e.to_str()).unwrap_or("").to_string();
                    let fname = path.as_ref().and_then(|p| p.file_name()).and_then(|n| n.to_str()).unwrap_or("").to_string();
                    
                    let engine = self.language_registry.resolve_with_content(&fname, &ext, "");

                    let mut smart_enter_str = None;
                    let (mut leading_whitespace, extra_indent) = if let Some(line) = self.state.buffer_mut().lines().get(cursor.row) {
                        let ws = line.chars().take_while(|c| c.is_whitespace()).collect::<String>();
                        ...
                        if trimmed.ends_with('{') || trimmed.ends_with('[') || trimmed.ends_with('(') {
                            ...
                            if trimmed.ends_with("then") || trimmed.ends_with("do") || trimmed.ends_with("else") || trimmed.ends_with('\\') {
                                extra = 4;
                            }
                ...
                KeyCode::Up => {
                    if !self.cached_completions.is_empty() {
                        if self.state.autocomplete_selected_index > 0 {
                            ...
                            if self.state.autocomplete_selected_index < self.state.autocomplete_scroll_offset {
                                self.state.autocomplete_scroll_offset = self.state.autocomplete_selected_index;
                            }
                            self.state.dirty.mark_full();
                        }
                    } else {
                ...
                KeyCode::Down => {
                    if !self.cached_completions.is_empty() {
                        if self.state.autocomplete_selected_index + 1 < self.cached_completions.len() {
                            self.state.autocomplete_selected_index += 1;
                            let max_visible = 10;
                            if self.state.autocomplete_selected_index >= self.state.autocomplete_scroll_offset + max_visible {
                                self.state.autocomplete_scroll_offset = self.state.autocomplete_selected_index - max_visible + 1;
                            }
                            self.state.dirty.mark_full();
                        }
                    } else {
                ...
                KeyCode::End => {
                    if !self.cached_completions.is_empty() {
                        self.state.autocomplete_selected_index = self.cached_completions.len().saturating_sub(1);
                        let max_visible = 10;
                ...
                KeyCode::PageDown => {
                    if !self.cached_completions.is_empty() {
                        let max_visible = 10;
                        self.state.autocomplete_selected_index = (self.state.autocomplete_selected_index + max_visible).min(self.cached_completions.len().saturating_sub(1));
                        if self.state.autocomplete_selected_index >= self.state.autocomplete_scroll_offset + max_visible {
                            self.state.autocomplete_scroll_offset = self.state.autocomplete_selected_index.saturating_sub(max_visible - 1);
                        }
                        self.state.dirty.mark_full();
                    } else {
                        for _ in 0..10 {
                ...
                KeyCode::PageUp => {
                    if !self.cached_completions.is_empty() {
                        let max_visible = 10;
                        self.state.autocomplete_selected_index = self.state.autocomplete_selected_index.saturating_sub(max_visible);
                        if self.state.autocomplete_selected_index < self.state.autocomplete_scroll_offset {
                            self.state.autocomplete_scroll_offset = self.state.autocomplete_selected_index;
                        }
                        self.state.dirty.mark_full();
            ...
            EditorMode::Command => match code {
                ...
                KeyCode::Char(ch) => {
                    ...
                    self.state.engine.set_status(format!(":{}", self.state.command_input));
                    self.state.dirty.mark_status();
                }
                KeyCode::Backspace => {
    ...
    fn next_search_match(&mut self) {
        ...
        let next = self.state.search_matches.iter()
    ...
    fn sync_buffer_cursor(&mut self) {
        let cursor = self.state.cursor(); self.state.buffer_mut().set_cursor_position(cursor.row, cursor.col);
    }

    /// Helper to get line length safely.
    fn line_length(&mut self, row: usize) -> usize {
        self.state.buffer_mut().line(row).map(|s| s.len()).unwrap_or(0)
    }
        ...
        match cmd {
            "w" => {
                ...
                if let Some(path) = path_opt {
                    ...
                    self.state.engine.set_status("Error: No file name (use :w <filename>)".to_string());
                }
            }
            "q" => {
                if self.state.document_mut().is_modified {
                    self.state.engine.set_status("Error: No write since last change (use :q! to override)".to_string());
                } else {
                    return Some(ShutdownReason::UserRequested);
                }
            }
            ...
            "fmt" => {
                ...
                if let Some(formatted) = engine.format(&text) {
                    ...
                    self.state.engine.set_status("Formatted current file.".to_string());
                    self.state.dirty.mark_full();
                } else {
                    self.state.engine.set_status("No formatting changes needed or formatter not available.".to_string());
                }
            }
            ...
            _ => {
                self.state.engine.set_status(format!("Unknown command: :{}", cmd));
            }
        }
        self.state.dirty.mark_status();
        None
