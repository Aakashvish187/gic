...
impl TreeSitterHighlighter {
    ...
    fn map_capture_to_token_kind(capture_name: &str) -> TokenKind {
        match capture_name {
            "keyword" | "keyword.function" | "keyword.return" | "keyword.operator" | "keyword.control" => TokenKind::Keyword,
            ...
            while let Some(m) = matches.next() {
                ...
                for row in start_pos.row..=end_pos.row {
                    ...
                    if start_byte_in_line < end_col {
                        ...
                        let token = HighlightedToken::new(token_kind, start_byte_in_line, end_col, token_text);
            ...
            for (row, tokens) in line_tokens.iter_mut() {
                ...
                for token in tokens.iter() {
                    ...
                    if token.start > current_offset {
                        ...
                        full_tokens.push(HighlightedToken::plain(current_offset, token.start, text));
                    }
                ...
                if current_offset < line_str.len() {
                    ...
                    full_tokens.push(HighlightedToken::plain(current_offset, line_str.len(), text));
                }
