...
fn test_detect_intent_kubernetes() {
    assert_eq!(
        detect_intent(Path::new("deployment.yaml")),
        ProjectType::Kubernetes
    );
    assert_eq!(
        detect_intent(Path::new("k8s/service.yml")),
        ProjectType::Kubernetes
    );
}
...
fn test_detect_intent_docker() {
    ...
    assert_eq!(
        detect_intent(Path::new("Dockerfile.dev")),
        ProjectType::Docker
    );
}
...
fn test_detect_intent_docker_compose() {
    assert_eq!(
        detect_intent(Path::new("docker-compose.yml")),
        ProjectType::DockerCompose
    );
    assert_eq!(
        detect_intent(Path::new("docker-compose.override.yaml")),
        ProjectType::DockerCompose
    );
}
...
fn test_detect_intent_terraform() {
    ...
    assert_eq!(
        detect_intent(Path::new("variables.tf")),
...
fn test_detect_intent_ansible() {
    assert_eq!(
        detect_intent(Path::new("playbook.yml")),
        ProjectType::Ansible
    );
...
fn test_detect_intent_github_actions() {
    assert_eq!(
        detect_intent(Path::new(".github/workflows/ci.yml")),
