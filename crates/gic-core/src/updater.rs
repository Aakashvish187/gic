    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            UpdateError::NetworkError(msg) => {
                write!(f, "Network error during update check: {}", msg)
            }
            UpdateError::AlreadyLatest(ver) => {
                write!(f, "GIC is already up to date (version {})", ver)
            }
...
impl Updater {
    ...
    pub fn perform_update(&self) -> Result<(), UpdateError> {
        ...
        if status.updated() {
            ...
            println!(
                "You are running the latest version of GIC (v{}). No update needed.",
