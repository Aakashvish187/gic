...
pub fn check_spelling(content: &str, lang: &str) -> Vec<EngineDiagnostic> {
    ...
    for (row_idx, line) in content.lines().enumerate() {
        ...
        let is_comment = match lang {
            "yaml" | "bash" | "dockerfile" | "python" | "ruby" => {
                line.trim_start().starts_with('#')
            }
            "rust" | "c" | "cpp" | "js" | "ts" | "java" | "go" | "csharp" => {
                line.trim_start().starts_with("//")
            }
        ...
        for &(typo, correction) in COMMON_TYPOS {
            ...
            while let Some(idx) = lower_line[start..].find(typo) {
                ...
                let is_start_boundary = start + idx == 0
                    || !lower_line.as_bytes()[start + idx - 1].is_ascii_alphabetic();
                let end_idx = start + idx + typo.len();
                let is_end_boundary = end_idx == lower_line.len()
                    || !lower_line.as_bytes()[end_idx].is_ascii_alphabetic();
